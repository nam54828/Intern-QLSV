import 'package:flutter/material.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';
import 'package:google_fonts/google_fonts.dart';

class AppText extends TextStyle {
  static TextStyle get text6 => TextStyle(
        fontSize: 6.sp,
        fontStyle: FontStyle.normal,
        fontWeight: FontWeight.normal,
        color: Colors.black,
      );

  static TextStyle get text8 => TextStyle(
        fontSize: 8.sp,
        fontStyle: FontStyle.normal,
        fontWeight: FontWeight.normal,
        color: Colors.black,
      );

  static TextStyle get text9 => TextStyle(
        fontSize: 8.sp,
        fontStyle: FontStyle.normal,
        fontWeight: FontWeight.normal,
        color: Colors.black,
      );

  static TextStyle get text10 => TextStyle(
        fontSize: 10.sp,
        fontStyle: FontStyle.normal,
        fontWeight: FontWeight.normal,
        color: Colors.black,
      );

  static TextStyle get text11 => TextStyle(
        fontSize: 11.sp,
        fontStyle: FontStyle.normal,
        fontWeight: FontWeight.normal,
        color: Colors.black,
      );

  static TextStyle get text12 => TextStyle(
        fontSize: 12.sp,
        fontStyle: FontStyle.normal,
        fontWeight: FontWeight.normal,
        color: Colors.black,
      );

  static TextStyle get text14 => TextStyle(
        fontSize: 14.sp,
        fontStyle: FontStyle.normal,
        fontWeight: FontWeight.normal,
        color: Colors.black,
      );

  static TextStyle get text15 => TextStyle(
        fontSize: 15.sp,
        fontStyle: FontStyle.normal,
        fontWeight: FontWeight.normal,
        color: Colors.black,
      );

  static TextStyle get text16 => TextStyle(
        fontSize: 16.sp,
        fontStyle: FontStyle.normal,
        fontWeight: FontWeight.normal,
        color: Colors.black,
      );

  static TextStyle get text17 => TextStyle(
        fontSize: 17.sp,
        fontStyle: FontStyle.normal,
        fontWeight: FontWeight.normal,
        color: Colors.black,
      );

  static TextStyle get text18 => TextStyle(
        fontSize: 18.sp,
        fontStyle: FontStyle.normal,
        fontWeight: FontWeight.normal,
        color: Colors.black,
      );

  static TextStyle get text20 => TextStyle(
        fontSize: 20.sp,
        fontStyle: FontStyle.normal,
        fontWeight: FontWeight.normal,
        color: Colors.black,
      );

  static TextStyle get text22 => TextStyle(
        fontSize: 22.sp,
        fontStyle: FontStyle.normal,
        fontWeight: FontWeight.normal,
        color: Colors.black,
      );

  static TextStyle get text30 => TextStyle(
        fontSize: 30.sp,
        fontStyle: FontStyle.normal,
        fontWeight: FontWeight.normal,
        color: Colors.black,
      );

  static TextStyle get text36 => TextStyle(
        fontSize: 36.sp,
        fontStyle: FontStyle.normal,
        fontWeight: FontWeight.w700,
        color: Colors.black,
      );

  static TextStyle get text40 => TextStyle(
        fontSize: 40.sp,
        fontStyle: FontStyle.normal,
        fontWeight: FontWeight.w700,
        color: Colors.black,
      );

  static TextStyle get textPoppin12 => GoogleFonts.poppins(
    fontSize: 12.sp,
    fontStyle: FontStyle.normal,
    fontWeight: FontWeight.w400,
    color: Colors.black
  );

  static TextStyle get textPoppin8 => GoogleFonts.poppins(
    fontSize: 8.sp,
    fontStyle: FontStyle.normal,
    fontWeight: FontWeight.w400,
    color: Colors.black
  );

  static TextStyle get textPoppin16 => GoogleFonts.poppins(
      fontSize: 16.sp,
      fontStyle: FontStyle.normal,
      fontWeight: FontWeight.w400,
      color: Colors.black
  );
}
